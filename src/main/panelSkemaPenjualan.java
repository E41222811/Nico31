...
public class panelSkemaPenjualan extends javax.swing.JPanel {
    ...
    private void setData() {
        ...
        //ResultSet result = statement.executeQuery("SELECT DATE_FORMAT(c.tanggal_transaksi,'%M')as Tanggal,SUM(data_transaksi.total_harga) AS Omset, SUM(b.nominal) AS Pengeluaran,(SUM(data_transaksi.total_harga)-SUM(b.nominal)) AS Keuntungan from data_transaksi join data_pengeluaran b join laporan_transaksi c on c.id_transaksi = data_transaksi.id_transaksi  GROUP BY DATE_FORMAT(Tanggal,'%m%Y') ORDER BY Tanggal DESC LIMIT 7");
            while (result.next()) {
    ...
    private void initComponents() {
        ...
        add(button1, new org.netbeans.lib.awtextra.AbsoluteConstraints(880, 30, 230, 60));
        ...
        add(panelTransparent1, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 110, 1110, 530));
