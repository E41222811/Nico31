...
public class panelSkemaPenjualan extends javax.swing.JPanel {
    ...
    private void setData() {
        try {
            ...
            //ResultSet result = statement.executeQuery("SELECT DATE_FORMAT(tanggal_transaksi,'%M') AS Tanggal, SUM(omset) AS Omset, SUM(pengeluaran) AS Pengeluaran, SUM(keuntungan) AS Keuntungan FROM laporan_transaksi GROUP BY DATE_FORMAT(tanggal,'%m%Y') ORDER BY tanggal DESC LIMIT 7");
            ResultSet result = statement.executeQuery("SELECT DATE_FORMAT(tanggal_transaksi,'%M') AS Tanggal, SUM(data_transaksi.total_harga) as Omset, SUM(b.nominal) as Pengeluaran,(SUM(data_transaksi.total_harga)-SUM(b.nominal)) as Keuntungan from data_transaksi join data_pengeluaran b ");
            while (result.next()) {
